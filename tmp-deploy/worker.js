...
export default {
  async fetch(request, env, ctx) {
    ...
    // Handle GET on /mcp — unauthenticated "streamable HTTP" GET is not supported.
    // Historically we returned 200 with a short text/event-stream body; SSE clients treated
    // the immediate close as a dropped connection and reconnected instantly, producing
    // 1+ rps per-client reconnect loops that burned our free-tier request quota (~145k/day
    // on 2026-04-23 across three misconfigured Cursor/CherryStudio clients alone).
    // Returning 405 per HTTP semantics tells conforming SSE clients — per the EventSource
    // spec "any 4xx response terminates the connection and must not reconnect" — to stop.
    // Retry-After 86400 gives a hint to non-conforming retry loops to back off for a day.
    if (!authHeader && request.method === "GET" && (subPath === "/mcp" || subPath === "/sse")) {
      return new Response(JSON.stringify({
        error: "GET on /mcp requires Authorization: Bearer <apify-token>. Unauthenticated GET is not supported — use POST with a bearer token for JSON-RPC method calls.",
